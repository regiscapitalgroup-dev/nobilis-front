...
import { deleteExperience, getDurations, getExperiences } from '../../../services/teamExperienceService';
...
interface PropsSectionDraft {
}

const SectionDraft: FC<PropsSectionDraft> = ({ }) => {
    ...
    const getDataExperience = async (page:number,offset:number=0) => {
        const experiences = await getExperiences({ limit, offset, status: EXPERIENCE_STATUS.DRAFT });
    ...
    const handlePageChange = async (page: number) => {
        ...
        await getDataExperience(1,offset);
    };
    ...
    const handleToDelete = async (id:number) => {
        let response = await confirm({ title: 'Delete Draft', message: 'This Draft Will Be Permanently Deleted. Continue?' });
        if (response) {
            let deleted = await deleteExperience(id);
            showErrorAlert({ title: 'Deleted', message: 'Experience deleted' });
            await getDataExperience(1);
        }
    }

    return (
        <div className="tap-host-secction-experiences-past-wrapper">
            <div className={`tap-host-section ${listExperiences.length > 0 ? '' : 'd-none'}`}>
                <div className='w-100 d-flex flex-row justify-content-between'>
                    <div className="tap-add-experience-2-btn-main" onClick={()=>history.goBack()}>
                ...
                <div className="tap-host-description-group">
                    <div className="tap-host-description-block">
                        <div className="tap-host-description-title">Continue Crafting Your Experiences</div>
                        <div className="tap-host-description-text">
                            Here you’ll find your saved experiences—exclusive invitations in progress. Refine, complete, or withdraw them before sharing with the Nobilis community.
                        </div>
                    </div>
                </div>

            </div>
            <div className={`tap-host-secction-experiences-past-list ${listExperiences.length == 0 ? '' : 'd-none'}`}>
                <div className="tap-host-description-text">
                        ...
                        <div className="tap-host-secction-experiences-past-details">
                            ...
                            <div className="tap-host-secction-experiences-past-actions">
                                ...
                                <div className="tap-host-secction-experiences-past-buttons">
                                    <div role='button' onClick={()=>handleToDelete(exp.id)} className="tap-host-secction-experiences-past-btn-view cursor-pointer">
                                        <span>Delete</span>
                                    </div>
                                    <div role='button' onClick={()=>goto(`/experience/edit/${exp.id}`)} className="tap-host-secction-experiences-past-btn-recreate cursor-pointer">
                                        Continue
                                    </div>
                                </div>
                            </div>
