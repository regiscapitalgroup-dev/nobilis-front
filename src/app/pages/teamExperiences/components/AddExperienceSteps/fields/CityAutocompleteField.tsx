  ...
  return (
    <div className='nb-language-autocomplete'>
      <div className='nb-language-wrapper'>
        <input className='nb-language-input' value={field.value || ''}
          onChange={(e) => {
      ...
      {open && (
        <div className='nb-language-menu'>
          ...
          {!loading && !error && visible.length > 0 && (
            <>
              {visible.map((label) => (
                <div key={label} className='nb-language-item d-flex flex-row justify-content-start align-items-center' onMouseDown={(e) => e.preventDefault()}
                  onClick={() => {
                    helpers.setValue(label)  
                    setQuery(label)
                    setOpen(false)
                  }}>
                  <div className='tap-add-experience-2-city-autocomplete-icon tap-flex-center mr_2'>
                    <KTSVG path='/media/svg/nobilis/teams_and_partner/location-icon.svg' />
                  </div>
