...
import { getBeneficiaries } from '../../../../../services/teamExperienceService'
...
export function BeneficiaryAutocompleteField({
    values,
    onChange,
    ...
    useEffect(() => {
        const fetchData = async () => {
            let beneficiaries_ = await getBeneficiaries();
            setBeneficiaries(beneficiaries_.results);
        }
        fetchData();
    },[])
    ...
    const visible = useMemo(() => {
        if (!enableSearch) return beneficiaries.slice(0, 10)

        const q = debounced.toLowerCase()
        return beneficiaries
            .filter((c) => c.name.toLowerCase().includes(q))
            .slice(0, 10)
    }, [enableSearch, debounced,beneficiaries])
    ...
    const selectedObjects = values
        .map((id) => beneficiaries.find((i) => i.id === id))
        .filter(Boolean)
